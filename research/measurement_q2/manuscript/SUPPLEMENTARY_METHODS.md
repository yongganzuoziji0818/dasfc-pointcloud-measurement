...
For a fixed registered group, let $R_1,\ldots,R_{n_d}$ be the calibration scan-pair maximum scores and let $R_{n_d+1}$ be the next scan-pair maximum score, computed with the same frozen estimator, valid-location rule, scale, and shrinkage coefficient. Exchangeability makes the rank of $R_{n_d+1}$ among the $n_d+1$ scores uniform up to conservative handling of ties. With $q_d=R_{(k_d)}$ from the calibration scores, the event $R_{n_d+1}\le q_d$ therefore has probability at least $k_d/(n_d+1)$. Because $R_{n_d+1}$ is the maximum standardized absolute error over every frozen valid location, this scalar event is exactly simultaneous containment of the complete field. Since $k_d=\lceil(n_d+1)(1-\alpha)\rceil$, the lower bound is at least $1-\alpha$. For $n_d=150$ and $\alpha=0.05$, it is $144/151=0.9536$. The proof is group-specific and gives no guarantee for the pooled fallback assigned to unregistered metadata.
...
## S11. Post-confirmatory P4 audit

P4 was frozen only after the primary synthetic, robustness, public-data, and Rockfall D5 results were complete. Its input manifest locked the formal configuration and report, Rockfall protocol, D5 target table and physical report, and frame-mapping report and residuals. Attempt 01 stopped before creating a run directory because its wrapper checked a nonexistent historical sentinel; the bootstrap log was retained. Attempt 02 changed only that precondition to the already frozen formal-report SHA-256 and then passed resource, execution, schema, reconstruction, and manifest checks.

The truth-free domain audit used seven observable quantities divided where appropriate by a robust scan-pair diagonal: median and 95th-percentile predicted scale, median and 95th-percentile match distance, mean support probability, valid fraction, and 95th-percentile absolute predicted normal displacement. Synthetic centres and scales were the median and 1.4826 times median absolute deviation across 240 known-domain tuning pairs. For each Rockfall event, P4 reported the root-mean-square robust standardized distance, nearest synthetic robust distance, empirical feature percentiles, and count outside the synthetic 1st--99th percentile envelope. None uses Rockfall displacement truth, and none restores a conformal guarantee.

For target $t$ and event $e$, the target-specific reference tolerance was the sum of the norms of that target's projected frame residuals at the two epochs. The alternative event-level tolerance was the root-sum-square of the two epoch mapping RMS residuals. A prediction interval *intersected* a reference range when their intersection was nonempty and *fully contained* it when both range endpoints lay inside the prediction interval. Counts were aggregated only at the event level after requiring all four targets. These tolerances are sensitivity constructions, not calibrated reference uncertainty or a GUM budget.

The field illustration was selected without manual case choice: lexicographically first among cases with pointwise marginal coverage at least 0.90, pointwise simultaneous failure, and DAS-FC simultaneous success. P4 reconstructed the frozen scale model and required stored and recomputed field metrics to agree within $10^{-9}$. The selected panel is an explanatory example and is not used for prevalence, effect, or coverage inference.
